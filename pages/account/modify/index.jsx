...
export default function AccountModify() {
    const 
        [ img, setImg ] = useState( "/img/user/user1.svg" ), 
        [ loader, setLoader ] = useState( false ),
        [ error, setError ] = useState( "" );
    return (
        <AccountRoot id="modify" icon="clipboard-check" title="Modifier">
            <div className="d-flex justify-content-center mx-2 mx-sm-3 mx-md-4 mb-4 field profil-container">
                ...
                <div className="profil-img-container d-flex justify-content-center align-items-center">
                    <img src={ img } alt="profil image" className="img profil-img" />
                    <input type="file" name="file" id="file" className="d-none" onChange={ e => {
                        const 
                            item = e.target.files[ 0 ],
                            reader = new FileReader();
                                reader.onload = function() {
                                    return setImg( this.result );
                                };
                        return reader.readAsDataURL( item );
                    } } />
                    <label htmlFor="file" className="profil-img-icon d-flex justify-content-center align-items-center">
                        <i className="bi bi-camera2"></i>
                    </label>
                </div>
            </div>
            <div className="w-100 py-3 field-list">
                <Field type="text" placeholder="about" name="A propos de moi" value="Nom de l'utilisateur" />
                <Field type="text" placeholder="name" name="Nom" value="Nom de l'utilisateur" />
                <Field type="email" placeholder="email" name="Email" value="Email de l'utilisateur" />
                <Field type="text" placeholder="profession" name="Profession" value="Profession de l'utilisateur" />
